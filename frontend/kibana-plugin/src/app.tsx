...
const getImpactSummary = (incident: Incident | null): ImpactSummary | null => {
  ...
  const manualMinutes = 75;
  const assistedMinutes = Math.max(10, 20 + automatedSteps * 3 - topConfidence * 8);
  const savedMinutes = Math.max(0, Math.round(manualMinutes - assistedMinutes));

  return {
    confidencePct: Math.round(topConfidence * 100),
    automatedSteps,
    manualMinutes,
    assistedMinutes: Math.round(assistedMinutes),
    savedMinutes,
  };
};
...
export const DataPulseApp = () => {
  ...
  const impact = getImpactSummary(incident);
                  ...
                  {impact && (
                    <>
                      <EuiPanel hasShadow={false} style={{ background: '#1D1E24', border: '1px solid #343741' }}>
                        <EuiFlexGroup alignItems="center" justifyContent="spaceBetween">
                          <EuiFlexItem>
                            <EuiTitle size="xs"><h3>Impact Summary</h3></EuiTitle>
                            <EuiText size="xs" color="subdued">Estimated value generated by DataPulse agents for this incident.</EuiText>
                          </EuiFlexItem>
                          <EuiFlexItem grow={false}>
                            <EuiBadge color="accent">Hackathon KPI</EuiBadge>
                          </EuiFlexItem>
                        </EuiFlexGroup>
                        <EuiSpacer size="m" />
                        <EuiFlexGroup gutterSize="m">
                          <EuiFlexItem>
                            <EuiStat
                              title={`${impact.savedMinutes}m`}
                              description="Time Saved"
                              titleColor="success"
                              textAlign="left"
                            />
                            <EuiText size="xs" color="subdued">
                              Manual baseline: {impact.manualMinutes}m • Agent-assisted: {impact.assistedMinutes}m
                            </EuiText>
                          </EuiFlexItem>
                          <EuiFlexItem>
                            <EuiStat
                              title={`${impact.confidencePct}%`}
                              description="RCA Confidence"
                              titleColor="accent"
                              textAlign="left"
                            />
                            <EuiProgress value={impact.confidencePct} max={100} size="xs" color="accent" />
                          </EuiFlexItem>
                          <EuiFlexItem>
                            <EuiStat
                              title={impact.automatedSteps}
                              description="Steps Automated"
                              titleColor="primary"
                              textAlign="left"
                            />
                            <EuiText size="xs" color="subdued">Resolver actions discovered and orchestrated in flow.</EuiText>
                          </EuiFlexItem>
                        </EuiFlexGroup>
                      </EuiPanel>
                      <EuiSpacer size="m" />
